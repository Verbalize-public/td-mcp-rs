...
impl ReconnectConfig {
	...
	pub fn from_env_vars(
		...
		Self {
			...
			probe_max: parse_ms(probe_max, defaults.probe_max, "TDMCP_RECONNECT_PROBE_MAX_MS"),
		}
	}
}
...
mod tests {
	...
	fn classify_transport_errors() {
		...
		assert!(is_transport_error(&ServiceError::TransportSend(
			rmcp::transport::DynamicTransportError::from_parts(
				"test",
				TypeId::of::<()>(),
				Box::new(std::io::Error::new(std::io::ErrorKind::ConnectionReset, "x")),
			)
		)));
	}
	...
	fn classify_non_transport_errors() {
		...
		assert!(!is_transport_error(&ServiceError::Cancelled { reason: None }));
