...
class InspectGeometryTest(unittest.TestCase):
    def test_pop_geometry_counts_and_attrs(self) -> None:
        class Attr:
            def __init__(self, name: str, size: int, typ: str = "float") -> None:
                self.name = name
                self.size = size
                self.type = typ

        node = SimpleNamespace(
            path="/project1/sphere1",
            family="POP",
            opType="spherePOP",
            children=[],
            pars=lambda: [],
            errors=lambda: "",
            warnings=lambda: "",
            numPoints=lambda: 252,
            numPrims=lambda: 500,
            pointAttributes=[Attr("P", 3), Attr("N", 3)],
            primAttributes=[],
            vertAttributes=[],
        )
        out = tdmcp_bridge.build_inspect_node(
            node, want_nodes=False, want_geometry=True
        )
        self.assertEqual(out["geometry"]["numPoints"], 252)
        self.assertEqual(out["geometry"]["numPrims"], 500)
        names = [a["name"] for a in out["geometry"]["attributes"]]
        self.assertEqual(names, ["P", "N"])
        self.assertEqual(out["geometry"]["attributes"][0]["domain"], "point")

    def test_sop_num_points_property(self) -> None:
        node = SimpleNamespace(
            path="/project1/sop1",
            family="SOP",
            opType="sphereSOP",
        ...
        self.assertEqual(out["geometry"]["numPoints"], 722)
        self.assertEqual(out["geometry"]["numPrims"], 100)

    def test_top_omits_geometry(self) -> None:
        node = _fake_node([])
        node.family = "TOP"
        node.opType = "nullTOP"
        out = tdmcp_bridge.build_inspect_node(
