...
and {{ skill("primer/editor-and-layout") }} for layout hygiene.
...
Style-guide convention: {% raw %}`{operator}_{descriptor}{#}`{% endraw %}, lowercase with underscores —
...
- **Stateful subsystems must have a public reset signal.** Route the project's
  ...
  design alongside data flow. Full contract: {{ skill("reset-state") }}.
...
- **Component boundary API (split by kind):**
  ...
  {{ skill("component-checklist") }} for the full reusable-COMP checklist.
...
- **Wiring that works is not done.** Once all logic is verified, do one final
  ...
  overlap avoidance are restated below; deepen with {{ skill("primer/editor-and-layout") }}
...
**Canonical:** {{ skill("network-design") }} 
