...
Reusable COMP boundary / About / reuse audit. Router: {{ skill("operate") }}.
Custom pars depth: {{ skill("custom-parameters") }}.
...
- API / naming / snippets (live-verified): {{ skill("custom-parameters") }}.
...
Rule + preference order: {{ skill("network-design") }} relative refs.
...
- Wrong relative form: bare name used where a **child** is meant (`./…`), or `./…`
  used where a **sibling** is meant (bare name) — see {{ skill("network-design") }}
...
**Canonical:** {{ skill("component-checklist") }} 
