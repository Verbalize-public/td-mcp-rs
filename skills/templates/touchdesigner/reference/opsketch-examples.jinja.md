...
Worked before → after pairs. Raw surfaces are fabricated but schema-faithful; OpSketch follows {{ skill("opsketch-notation") }} + {{ skill("opsketch-importance-gating") }}.
...
claim that the live node has one ({{ skill("node-comments") }}).
...
**Canonical:** {{ skill("opsketch-examples") }} 
