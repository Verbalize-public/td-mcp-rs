...
- For tool-managed sampling/recording and transport actions, use
  {{ skill("timed-capture") }}. Ordinary inspection never changes play state.
- For repeatable stateful checks, use the public reset signal and advance
  sequentially with verified cooking ({{ skill("reset-state") }}). Timeline
...
- Sequential bridged calls still apply: {{ skill("tooling-concurrency") }}.
