...
from the sequential-bridged-call gate ({{ skill("tooling-concurrency") }}).
...
- Start / stop TD to verify a packed project: {{ skill("lifecycle") }}
- Compat dialogs after repacking across builds: {{ skill("popups") }}
- What `.toe` / `.tox` are: {{ skill("primer/tox-toe-components") }}
- Component packaging before you pack: {{ skill("component-checklist") }}

---

**Canonical:** {{ skill("project-io") }}
