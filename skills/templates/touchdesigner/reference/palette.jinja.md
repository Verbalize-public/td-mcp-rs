...
the descriptions is a separate, deliberate pass — {{ skill("palette-scan") }}.

`palette_index` is offline: no `pid`, no bridge, exempt from the sequential-
bridged-call gate ({{ skill("tooling-concurrency") }}).
...
1. `inspect` the placed COMP with `include: ["params"]`. Its **custom parameters
   are its API**; drive those, never its internals ({{ skill("custom-parameters") }}).
...
- Building the descriptions, and the probe blacklist: {{ skill("palette-scan") }}
- Driving a component through its custom pars: {{ skill("custom-parameters") }}
- In/Out boundaries and reuse hygiene: {{ skill("component-checklist") }}
- What `.tox` files are: {{ skill("primer/tox-toe-components") }}
- Sketching the placed component: {{ skill("opsketch-notation") }}

---

**Canonical:** {{ skill("palette") }}
