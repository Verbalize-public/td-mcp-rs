...
Most setups have more than one computer: the design machine, a render node,
the show machine in the rack. Federation lets one assistant, in one
conversation, address all of them.
...
Every machine runs the same daemon. One takes the **master** role; the others
join it.
...
- `fleet` returns every TouchDesigner on every machine, each row tagged with
  the `daemonId` and `hostname` that owns it.
- Any tool call carrying that `daemonId` is proxied to the right machine and
  comes back with the same shape it would have locally.
...
| | |
| --- | --- |
| **Same LAN** | Federation is for a local network — a studio, a venue, a rack. Do not expose it to the open internet. |
...
| **A flat subnet** | The dashboard scans your subnet and finds the machines for you. Machines on a different subnet have to be added by IP. |
| **A passphrase (optional, recommended)** | Federation works with no authentication, so a trusted studio LAN needs zero setup. On any network you don't control, set a PSK. See [Security model](#security-model). |
...
on the master you can skip to the next step.
...
2. Click **Scan network**. The master sweeps your subnet on the daemon port
   and lists every td-mcp-rs it finds, with hostname, version and role.
...
already a master, then writes its federation settings — role, the master's
URL, and the key it should use to call home. Nothing to edit on the far
machine.

6. **Restart the slave daemon** so it picks the settings up. It registers with
   the master within seconds and its TouchDesigner processes appear in the
   fleet, grouped under its hostname.
...
`fleet` returns every process on every machine. Rows carry `daemonId` and
`hostname`, so the assistant can say *"three on studio-b, one here"*.
...
the call. The result is identical to a local one.
...
`spawn_td`, `kill_td`, `project_install_bridge` and the rest all proxy.

### Process ids across machines

Process ids are only unique per machine, so two machines can run a
TouchDesigner with the same pid. When that happens and the assistant didn't
say which machine it meant, the call fails immediately:
...
The default is convenience, so read this before you enable sharing.

**What protects you by default**

| Rule | Why it matters |
| --- | --- |
| Loopback-only by default | A fresh install is not on the network at all. You have to tick a box and restart. |
...
| Identity conflicts are rejected | Re-registering the same `daemonId` from a different host is refused with a diagnostic. |
...
- **Auth is off by default, and a network bind does not require it.** With
  sharing on and no PSK, anything that can reach port 9860 can list your
  projects, run Python inside TouchDesigner, and start or kill processes. Set
  a PSK on any network you don't control.
...
The full auth matrix and route allowlist is in
...
| **Scan network** | Re-sweep the subnet, after adding a machine or changing an IP. |

On a slave's own dashboard, **Go standalone** detaches it from the master and
returns it to single-machine operation.

Slaves that stop pushing their fleet are marked unreachable rather than
dropped, so a machine that's asleep or rebooting stays visible.

---

## Limits

- **One level deep.** A master's slaves cannot themselves be masters. No tree,
  no multi-master, no leader election.
...
- **The master is a single point of failure.** If it goes down, the slaves
  keep running TouchDesigner — you lose the single-seat view until it's back.
- **One assistant at a time per TouchDesigner.** The per-process exclusive
  ...
  at once, but not on the same TouchDesigner process.
...
- Every machine must be on the **same subnet** and have **Share on my
  network** ticked — a daemon still on `127.0.0.1` is invisible.
...
- Check `master_url` on the slave resolves from *its* side — the master
  advertises its own hostname, which some networks don't resolve. Use the
  master's IP address instead if so.
...
- On the slave: `tdmcp-daemon logs 100`, look for `federation`.
