...
| `inspect`                   | Structural read for explicit `paths[]` batch (nodes / params / errors / warnings / geometry); no auto-recursion | **Shipped**           |
| `capture`                   | Perception — `top` / `preview` / `auto` / `chop_data` / `pop_data` / `chop_image`† / `pop`† († aliases of preview)  | **Shipped**           |
...
| `capture`        | Image modes: `{ ok: true, path, bytes, mimeType, imageBase64?, maxSize?, mode?, family? }` + MCP image content when PNG present (`imageBase64` stripped from structured after promotion). `chop_data`: `{ ok: true, path, mode, family, numChans, numSamples, rate?, channels:[{name, samples}], truncation? }` (structured only; no image). `pop_data`: `{ ok: true, path, mode, family, numPoints, numPrims, numPointsReturned, attributes:[{name, domain?, size?, type?}], data:{attr:[[…],…]}, truncation? }` optional `attrs` (default `P`); caps 8 attrs / 256 points / 4096 scalars |
...
| non-empty      | allowlist only (`nodes` / `params` / `errors` / `warnings` / `geometry`) |


`params` is opt-in. `geometry` is opt-in (POP/SOP light card: `numPoints` / `numPrims` via safe method/property reads, attribute schema from `pointAttributes` / … — **no** point samples; use `capture` `pop_data` for samples). `errors` / `warnings` are string arrays from `OP.errors()` / `OP.warnings()` (no recurse). Not gated by `detailLevel`. When those sections are included (including via default empty `include`), empty arrays are still emitted as “section loaded” — they never flip MCP tool failure. Tool-level failure is only bridge soft-fail (`ok: false`), queue busy, or transport. Unknown `include` values are **rejected** at MCP arg deserialize. Non-POP/SOP nodes omit `geometry` when requested (no hard fail).
...
| `pop_data`   | **Shipped** | POP → capped columnar attribute samples (8 attrs, 256 points, 4096 scalars); optional `attrs` (default `P` when present); soft `truncation` + `tdmcp.perception.pop_truncated`; empty → `tdmcp.perception.empty_pop`; wrong family → `tdmcp.perception.wrong_family`; ignores `maxSize`. Alias `pop` remains preview — not samples |
...
| Result JSON policy | Before sizing/wire encode, `result` is coerced to a JSON-true tree: TD OPs → `{__td:"op",path,opType?}`; callables/methods → `{__td:"callable",name,hint}` (**never invoked**); other non-JSON → `{__td:"repr",…}`. Coercion failure → `tdmcp.script.result_not_serializable` (soft). Wire encode failure must **not** tear down the bridge IPC (`tdmcp.bridge.response_encode_failed` rewrite). |
...
| MCP item          | Hard code is bridge `code` when set (`tdmcp.script.too_large` / `tdmcp.script.result_too_large` / `tdmcp.script.result_not_serializable`), else `tdmcp.script.execution_failed`. `items[0].exception` carries the reduced report. `span.line` / `column` / `snippet` filled from `syntax` or last `<string>` frame. |
