...
| `dialogs` | `list`: `{ ok: true, pid, windowStatus, popups, accessibilityGranted?, permissionHint? }` (last two macOS only) · `describe`: `{ ok: true, pid, popup }` · `dismiss`: `{ ok: true, pid, dismissed, via, stillOpen }`. **macOS caveat:** TouchDesigner draws its own UI, so its dialogs usually expose no readable buttons and `popup.buttons` comes back empty; some (`THREAD CONFLICT`) cannot be dismissed at all and return `tdmcp.dialog.dismiss_failed` rather than faking success. Native dialogs TD opens are fully readable. |
...
| `palette_index` | Per action — `scan`: `{ ok, roots, added, updated, removed, stale, total, ignored, suspect?, suspectHint? }` · `list`: `{ ok, entries: [{ paletteId, name, category, source, summary?, tags?, cardStatus, probeStatus, probeMessage?, ignored?, thumb? }], total, truncation? }` · `get`: `{ ok, entry, toxPath, card?, cardError? }` · `describe`: `{ ok, paletteId, cardPath, bytes }` · `ignore`/`unignore`: `{ ok, ignore, affected }` · `forget`: `{ ok, removed, total }` · `stats`: `{ ok, total, described, stale, undescribed, failed, ignored, scannedAt, autoIgnoreAfter, byCategory }` |
...
**Operator comment.** Every ok node carries `comment` (from `OP.comment`) whenever the operator's comment is non-empty, **independent of `include`** — it is identity metadata alongside `path` / `family` / `opType`, not a section. Blank/whitespace-only comments and unreadable ones are omitted entirely (read failures never flip node or top-level `ok`). Capped at **1024** characters; when cut, the value ends in `…` and the node also carries `commentTruncated: true`. Written through `mutate_nodes` `create` / `set` `comment` (see below). Comments are authored metadata, never runtime evidence.
...
Either level adds `comment` to a child entry when that child's `OP.comment` is non-empty, capped at **160** characters (tighter than the node-level 1024 cap because a roster holds up to 256 of them; cut values end in `…` with no per-entry flag). This makes the parent inspect a one-call orientation pass: name, type, and each child's stated purpose.
