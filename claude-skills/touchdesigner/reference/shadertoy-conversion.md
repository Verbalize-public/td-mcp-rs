...
- [ ] Look claim graded via [`look-grade`](./look-grade.md) — capture compared with
...
- **Vertex+fragment pairs (materials):** these are GLSL MAT territory, not a GLSL TOP —
  positions go through `TDDeform()`/`TDWorldToProj()`; see [`td-glsl-ground-truth`](./td-glsl-ground-truth.md).

## Related

- [`glsl`](./glsl.md) — dialect workflow
- [`td-glsl-ground-truth`](./td-glsl-ground-truth.md) — sampler/res traps, GLSL MAT vs TOP
...
**Canonical:** [`shadertoy-conversion`](./shadertoy-conversion.md) 