...
`inspect`. Router: [`operate`](../SKILL.md).
...
| Python | `n.comment` (see [`python-api`](./python-api.md)) — use the `mutate_nodes` field instead unless you are already in a script |
...
`inspect` / `capture` ([`definition-of-done`](./definition-of-done.md)).
...
The gate is the same instinct as [`opsketch-importance-gating`](./opsketch-importance-gating.md):
...
- **No ceremony** — no dates, no author or agent signature, no changelog. Version
  and date belong on the COMP's About page ([`component-checklist`](./component-checklist.md)).
...
4. Carry the comments into your OpSketch (`# …` trailing form) so the sketch you
   ...
   [`opsketch-notation`](./opsketch-notation.md).
...
- [`network-design`](./network-design.md) — in-network documentation and layout conventions
- [`component-checklist`](./component-checklist.md) — About page, the COMP-level counterpart
- [`opsketch-notation`](./opsketch-notation.md) — carrying comments into a sketch
- [`python-api`](./python-api.md) — `n.comment` from a script
- [`definition-of-done`](./definition-of-done.md) — comments are metadata, not evidence


---

**Canonical:** [`node-comments`](./node-comments.md)