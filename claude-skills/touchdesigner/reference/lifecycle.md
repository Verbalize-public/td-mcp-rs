...
| `{ok:false, outcome:"wait_timeout", stillAlive, startupDialogs}` **with** `startupDialogs` non-empty | Dialogs were observed; a blocking modal is a possible cause | read them before choosing an action, then poll `fleet` — see [`popups`](./popups.md) |
