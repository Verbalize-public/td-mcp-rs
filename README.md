...
### An MCP server for TouchDesigner.

**Your assistant reads the running project, builds and wires nodes, runs it,
looks at the render, and fixes what it got wrong. On your machine, or across
every machine in the studio, with whatever assistant you already use.**
...
<table>
  <tr>
    ...
    <td width="50%" align="center"><img src="docs/screens/palette-tree.png" alt="Palette tab — browse your component library with previews and agent-ready cards" width="420"><br><sub><b>Palette</b> · your components, with reference cards for the agent</sub></td>
  </tr>
  <tr>
    ...
    <td width="50%" align="center"><img src="docs/screens/settings-dirty.png" alt="Settings tab — every setting with save gating" width="420"><br><sub><b>Settings</b> · every setting, with save gating</sub></td>
  </tr>
</table>

**[Quick start](#quick-start) · [What to ask for](#what-to-ask-for) · [Federation](#federation) · [Tools](#tools) · [Full install guide](docs/INSTALL.md)**
...
Ask an AI assistant about TouchDesigner today and it answers from memory. It
has never seen your network, cannot check whether the wire it suggested
exists, and does not know your render is black.

td-mcp-rs is a small tray app that runs beside TouchDesigner and gives your
assistant a live connection into the open project. It can read any operator,
create and wire nodes, drop in Palette components, run Python, screenshot a
TOP and look at the result, then correct itself.
...
No subscription, no account, no sign-in, no relay server. It speaks **MCP**,
so Claude Code, Cursor, Codex CLI, VS Code / Copilot, Windsurf, Zed, Cline and
the rest all work with the same one-line setup. Your AI bill stays between you
and whoever you already pay. MIT licensed.

### 2 · It drives TouchDesigner, not one open file

It can start TouchDesigner, open or create a project from your template, work
with several instances at once (every call names an OS process id), edit
`.toe`/`.tox` files offline through Derivative's own tools, inject the bridge
into an existing project, dismiss a modal dialog that would otherwise block
everything, and shut it all down afterwards.

### 3 · It scales past one computer

Run td-mcp-rs on your render nodes, media server and booth machine. Point one
**master** at them and a single assistant addresses the whole room: inspect a
node on `studio-b`, capture the output on `studio-c`, push the same fix to all
three. See [Federation](#federation).

### 4 · It knows TouchDesigner

The daemon ships an **operate manual**: primers on cooking and operator
families, network design conventions, custom parameters, GLSL ground truth,
Shadertoy porting, and a compact notation for sketching a network before
building it. Your assistant reads these on its own instead of improvising.

That works at any size of request:

| You want to… | Say something like |
| --- | --- |
| **Learn** | *"What's happening in this feedback loop? Walk me through it."* |
...
Each download is one self-contained binary — no runtime, no DLLs. The
one-click Windows installer (`.exe`) is coming; until then the manual steps
...
**macOS (tar.gz)** — or skip the bundle and put the bare binary on your `PATH`:
...
(or use the full path in the editor configs below):
...
Finish either path with one command — it unpacks the bridge and assets:
...
**From source** — needs [Rust](https://rustup.rs) (`rustup` installer, then
restart your terminal). This puts `tdmcp-daemon` on your `PATH`, which makes
every editor config below a single word:
...
**double-click** for the dashboard. After that your editor starts it whenever
it needs it.

### Step 2 · Connect your AI assistant

Every option below does the same thing: run `tdmcp-daemon` with the argument
`mcp`. Pick your tool:
...
with `cargo install`, the default `tdmcp-daemon` is already correct — press
enter. Otherwise paste the path from
[the install guide](docs/INSTALL.md#where-the-binary-lands).

This registers the MCP server **and** installs the TouchDesigner skill pack,
so Claude reaches for the right tool without being told.
...
Same server, different file. Per-tool walkthroughs with exact config paths are
in **[docs/INSTALL.md](docs/INSTALL.md#step-2--connect-your-assistant)**.

The shape for any MCP client:
...
Save the project and it stays there.

> To skip doing this by hand for every project, ask your assistant:
>
> *"install the tdmcp bridge into every .toe in this folder"* — that's the
> `project_install_bridge` tool, working offline with no TouchDesigner needed.

### Step 4 · Say hello

> *"Connect to my TouchDesigner and show me what's in my project."*

Your project appears in the dashboard's fleet, marked **connected**.

<details>
<summary><b>Under the hood</b> — what's running, headless mode, CLI</summary>

- Your editor spawns `tdmcp-daemon mcp`, a thin proxy: it makes sure the
  background service is up, then forwards MCP traffic to it on port `9860`.
  The service keeps running in the tray after your editor quits, so
  TouchDesigner never loses its connection.
- **Tray:** left-click = glance card, double-click = dashboard, right-click =
  menu (Dashboard · Stop · Restart). Closing a window only hides it; **Stop**
  (two-step confirm) is the real exit.
...
Every setting lives in one TOML file — see [`docs/CONFIG.md`](docs/CONFIG.md).
...
Real prompts, and the tools behind them.
...
batch. If step 4 fails, you're told which step and why, instead of being left
with half a network.

**Use the Palette instead of rebuilding it**
...
`palette_index` catalogues them, `palette_probe` opens them safely to read
their real interface, and `mutate_nodes place` drops one in and wires it in
the same batch. Nothing about your palette ships with this tool — it learns
yours, on your machine, a slice at a time, and you can blacklist anything you
...
or pulls raw CHOP channel data. The assistant looks at the actual pixels
instead of assuming its code worked.
...
the show machine in the rack. td-mcp-rs turns that into one addressable fleet.
...
configures the ones you pick. No config files to edit on the far end, no IP
hunting.

Your assistant then sees every TouchDesigner in the room in one `fleet` call,
and addresses any of them by machine + process id:
...
**Guard rails.** Federation is LAN-only and off until you enable it.
Destructive local actions — shutdown, restart, session admin — are not
reachable from the network at all. Authentication is a shared key you switch
on: optional, so a trusted studio LAN needs zero setup, and recommended
anywhere else. Setup, threat model and the auth matrix:
...
Every tool returns structured, catalog-backed diagnostics with a stable error
code and a fix hint, so a failure gives the assistant enough to repair itself.
...
| Tool | What the assistant can do |
| --- | --- |
| `mutate_nodes` | Create · set · wire · delete · place a Palette component — one ordered batch, with the failing step named |
...
| Tool | What the assistant can do |
| --- | --- |
| `spawn_td` / `kill_td` | Start and stop TouchDesigner, with a deterministic process id |
| `dialogs` | Find and dismiss the modal popups that block everything else |
| `project_unpack` / `project_pack` | Open a `.toe`/`.tox` as editable files and pack it back |
| `project_lint` | Check a project file before you take it to the venue |
| `project_install_bridge` | Inject or refresh the bridge inside any project, offline |
| `td_installs` | Every TouchDesigner on disk, and whether it's usable |
...
<details>
<summary><b>Fine print</b> — caps and conventions</summary>
...
- A timeout fails the *wait*, not the bridge — TouchDesigner is left alone to
  finish.
...
One page for daemon health, every TouchDesigner grouped by machine, which
assistants are connected, and what just happened.
...
| *Nothing connected yet — one click to reveal the `.tox`* | *Offline — a reason and only safe actions* |
| ![Logs](docs/screens/logs-filtered.png) | ![Settings](docs/screens/settings-dirty.png) |
| *Logs — filter, search, follow or pause* | *Settings — save gating and restart hints* |
| ![Add slave](docs/screens/modal-add-slave.png) | ![Stop confirmation](docs/screens/stop-confirm.png) |
| *Federation — add a machine, with a subnet scan* | *Destructive actions confirm twice* |
...
- **Local only.** Assistant ↔ daemon ↔ TouchDesigner all talk over loopback.
  No cloud relay, no account, no telemetry, no phone-home.
- **Off the network by default.** Nothing listens outside `localhost` until
  you tick a box and restart. Turn it on for a [fleet](#federation), and set a
  shared key unless you fully trust the network — the
  [threat model](docs/FEDERATION.md#security-model) spells out the trade.
- **Addressed by process id.** Every call names an OS pid. No sticky sessions,
  no hidden "current target".
...
- [x] Live inspect / mutate / Python / perception over a local bridge
...
- [ ] Linux / Wine support — TCP transport done; lifecycle and packaging next
...
| [`CONSTITUTION.md`](CONSTITUTION.md) | Rust engineering rules — never-panic, lints |
