...
| R5 | `BridgeSessions::spawn` same-pid supersede | Prior actor is cancelled via token; if the old IPC stream is still open, OS-level dual pipes can exist briefly until Python closes / idle-dead. | Generation-guarded teardown + cancel-in-await prevents dual *actors* from serving the registry pid. Full pipe close remains peer-owned. | 2026-08-03 |
